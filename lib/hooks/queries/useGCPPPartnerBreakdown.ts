...
/**
 * useGCPPPartnerBreakdown - React Query hook for GCPP Partner Breakdown data
 *
 ...
 *
 * @example
 * const { data, isLoading, error, refetch } = useGCPPPartnerBreakdown({
 *   date: '2025-01-15',
 *   team: ['WEB'],
 *   partner: ['Google'],
 * })
 */
export function useGCPPPartnerBreakdown(filters: Record<string, any>) {
  const { crossFilters } = useCrossFilter()
  const baseFilters = extractBaseFilters(filters, crossFilters)

  return useQuery({
    queryKey: queryKeys.gcppPartnerBreakdown(baseFilters),
    queryFn: async () => {
      const response = await fetch('/api/gcpp-check/partner-breakdown', {
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: Failed to fetch partner breakdown data`)
      }
      ...
      if (result.status === 'ok') {
        ...
        throw new Error(result.message || 'Unknown error fetching partner breakdown')
      }
    },
