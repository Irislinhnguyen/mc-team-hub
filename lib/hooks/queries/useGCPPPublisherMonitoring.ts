...
/**
 * useGCPPPublisherMonitoring - React Query hook for GCPP Publisher Monitoring data
 *
 ...
 *
 * @example
 * const { data, isLoading, error, refetch } = useGCPPPublisherMonitoring({
 *   date: '2025-01-15',
 *   team: ['WEB'],
 *   partner: ['Google'],
 * })
 */
export function useGCPPPublisherMonitoring(filters: Record<string, any>) {
  const { crossFilters } = useCrossFilter()
  const baseFilters = extractBaseFilters(filters, crossFilters)

  return useQuery({
    queryKey: queryKeys.gcppPublisherMonitoring(baseFilters),
    queryFn: async () => {
      const response = await fetch('/api/gcpp-check/publisher-monitoring', {
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: Failed to fetch publisher monitoring data`)
      }
      ...
      if (result.status === 'ok') {
        ...
        throw new Error(result.message || 'Unknown error fetching publisher monitoring')
      }
    },
