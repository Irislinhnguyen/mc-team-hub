...
export async function getNewSalesQueries(filters: Record<string, any>) {
  ...
  // Build WHERE clause for queries with JOIN (uses bp. alias to avoid ambiguity)
  const breakdownConditionsWithJoin: string[] = []

  if (filters.startDate && filters.endDate) {
    breakdownConditionsWithJoin.push(`(bp.start_date <= '${filters.endDate}' AND bp.end_date >= '${filters.startDate}')`)
  }

  if (filters.team) {
    if (Array.isArray(filters.team) && filters.team.length > 0) {
      const teamCondition = await buildTeamConditions(filters.team, 'bp.pic')
      if (teamCondition) {
        breakdownConditionsWithJoin.push(teamCondition)
      }
    } else if (filters.team !== '') {
      const teamCondition = await buildTeamCondition(filters.team, 'bp.pic')
      if (teamCondition) {
        breakdownConditionsWithJoin.push(teamCondition)
      }
    }
  ...
  if (filters.pic) {
    if (Array.isArray(filters.pic) && filters.pic.length > 0) {
      const values = filters.pic.map(v => `'${v}'`).join(', ')
      breakdownConditionsWithJoin.push(`COALESCE(nsm.pic, bp.pic) IN (${values})`)
    } else if (filters.pic !== '') {
      breakdownConditionsWithJoin.push(`COALESCE(nsm.pic, bp.pic) = '${filters.pic}'`)
    }
  }
  ...
  if (filters.pid) {
    if (Array.isArray(filters.pid) && filters.pid.length > 0) {
      const values = filters.pid.map(v => `'${v}'`).join(', ')
      breakdownConditionsWithJoin.push(`bp.pid IN (${values})`)
    } else if (filters.pid !== '') {
      breakdownConditionsWithJoin.push(`bp.pid = '${filters.pid}'`)
    }
  }
  ...
  if (filters.pubname) {
    if (Array.isArray(filters.pubname) && filters.pubname.length > 0) {
      const values = filters.pubname.map(v => `'${v}'`).join(', ')
      breakdownConditionsWithJoin.push(`bp.pubname IN (${values})`)
    } else if (filters.pubname !== '') {
      breakdownConditionsWithJoin.push(`bp.pubname = '${filters.pubname}'`)
    }
  }
  ...
  if (filters.month) {
    if (Array.isArray(filters.month) && filters.month.length > 0) {
      const values = filters.month.map(v => v).join(', ')
      breakdownConditionsWithJoin.push(`bp.month IN (${values})`)
    } else if (filters.month !== '') {
      breakdownConditionsWithJoin.push(`bp.month = ${filters.month}`)
    }
  }
  ...
  if (filters.year) {
    if (Array.isArray(filters.year) && filters.year.length > 0) {
      const values = filters.year.map(v => v).join(', ')
      breakdownConditionsWithJoin.push(`bp.year IN (${values})`)
    } else if (filters.year !== '') {
      breakdownConditionsWithJoin.push(`bp.year = ${filters.year}`)
    }
  }

  const breakdownWhereClauseWithJoin = breakdownConditionsWithJoin.length > 0 ? `WHERE ${breakdownConditionsWithJoin.join(' AND ')}` : ''

  // Build WHERE clause for site count details WITHOUT overlap filter
  // Frontend will filter by sales_month/cs_month instead
  // Only apply PIC/team filters if present
  const detailsConditions: string[] = []

  // Handle team filter (use nsm.pic alias for consistency with JOIN query)
  if (filters.team) {
    if (Array.isArray(filters.team) && filters.team.length > 0) {
      const teamCondition = await buildTeamConditions(filters.team, 'nsm.pic')
      if (teamCondition) {
        detailsConditions.push(teamCondition)
      }
    } else if (filters.team !== '') {
      const teamCondition = await buildTeamCondition(filters.team, 'nsm.pic')
      if (teamCondition) {
        detailsConditions.push(teamCondition)
      }
    }
  ...
  // Handle PIC filter (use nsm.pic alias)
  if (filters.pic) {
    if (Array.isArray(filters.pic) && filters.pic.length > 0) {
      const values = filters.pic.map(v => `'${v}'`).join(', ')
      detailsConditions.push(`nsm.pic IN (${values})`)
    } else if (filters.pic !== '') {
      detailsConditions.push(`nsm.pic = '${filters.pic}'`)
    }
  }
  ...
  return {
    ...
    // Returns raw site data with contract duration for frontend aggregation
    ...
    // NOTE: Frontend will filter by sales_month/cs_month and aggregate counts
    // This ensures counts match the Details table (which is the source of truth)
    salesCsSiteCounts: `
      WITH contract_duration AS (
        -- Calculate contract duration for each site
        -- Include team info for aggregation
        SELECT DISTINCT
          nsm.pid,
          nsm.pic,
          nsm.start_date,
          nsm.end_date,
          DATE_DIFF(nsm.end_date, nsm.start_date, DAY) as contract_duration_days,
          fs.team
    ...
    // NOTE: No overlap filter here - frontend filters by sales_month/cs_month instead
    // The Details table is the SOURCE OF TRUTH - it determines which sites to count
    salesCsSiteDetails: `
      SELECT DISTINCT
        nsm.pid,
        nsm.pubname,
