 ...
 */
export async function buildTeamCondition(teamId: string, picColumn: string = 'pic'): Promise<string> {
  ...
  return `${picColumn} IN (${picValues})`
}

/**
 * Build SQL WHERE conditions for multiple team filters
 */
export async function buildTeamConditions(teamIds: string[], picColumn: string = 'pic'): Promise<string> {
  console.log('[buildTeamConditions] Input team IDs:', teamIds, 'picColumn:', picColumn)
  ...
  const teamConditions = await Promise.all(
    teamIds.map(teamId => buildTeamCondition(teamId, picColumn))
  )
