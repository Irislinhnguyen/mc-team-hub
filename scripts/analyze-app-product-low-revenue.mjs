/**
 * Analyze app_ Product Publishers with Low Revenue (< $10)
 *
...
async function analyzeLowRevenuePublishers() {
  try {
    console.log('='.repeat(80))
    console.log('Analyzing app_ Product Publishers with Revenue < $10')
    console.log('Table: gcpp-check.GI_publisher.agg_monthly_with_pic_table')
    console.log('Product Filter: Starts with "app_"')
    console.log('Date Range: June 2025 onwards')
    ...
    const query = `
      WITH app_publishers_recent AS (
        -- Get all publishers with app_ products and their total revenue since June 2025
        SELECT
          pid,
          pubname,
      ...
      SELECT *
      FROM app_publishers_recent
      WHERE total_revenue < 10
      ORDER BY total_revenue DESC, pid
    `

    console.log('🔍 Querying BigQuery for publishers with revenue < $10...')
    console.log('-'.repeat(80))

    const rows = await BigQueryService.executeQuery(query)

    console.log(`✅ Found ${rows.length} publishers with app_ products and revenue < $10`)
    console.log()

    if (rows.length > 0) {
      const pidWidth = 10
      const pubnameWidth = 60
      const revenueWidth = 15

      console.log(`${'PID'.padEnd(pidWidth)} | ${'Pubname'.padEnd(pubnameWidth)} | ${'Revenue'.padEnd(revenueWidth)} | Products`)
      console.log('-'.repeat(80))
      ...
      rows.forEach(row => {
        ...
        console.log(`${pid} | ${pubname} | ${revenue} | ${products}`)
      })
      console.log()

      console.log('='.repeat(80))
      console.log('Summary:')
      console.log(`  Total publishers with app_ products and revenue < $10: ${rows.length}`)
      ...
      const csvFileName = 'app-product-publishers-low-revenue.csv'
      const csvFilePath = path.join(projectRoot, csvFileName)

      const csvHeader = 'PID,Pubname,Total_Revenue,Num_MIDs,Num_Products,Products\n'
      const csvRows = rows.map(row => {
        ...
        return `${pid},${pubname},${revenue},${numMids},${numProducts},${products}`
      }).join('\n')
      ...
      console.log('✅ No publishers found with app_ products and revenue < $10')
    }
